...
literal → metric_id → window/update_mode → source/as_of → owner
...
## PUMP market share
- `47%` → `pump.market_share.pct.current` → current/REPORT_SNAPSHOT → defillama/2026-08-25T18:35:17Z → CGPT_CURSOR (MAPPED_CANONICAL)
- `47% (row above)` → `pump.market_share.pct.current` → current/REPORT_SNAPSHOT → defillama Launchpad category dailyFees/UNKNOWN → CGPT_CURSOR (MAPPED_CANONICAL)
...
## SOL inflation
- `3.68% / burn thin` → `sol.inflation.pct.current` → current/REPORT_SNAPSHOT → Solana RPC + DefiLlama burn/2026-08-11T21:13:38.447286Z → CGPT_CURSOR (MAPPED_CANONICAL)
- `3.68%` → `sol.inflation.pct.current` → current/REPORT_SNAPSHOT → UNKNOWN/UNKNOWN → CGPT_CURSOR (MAPPED_CANONICAL)
...
## HYPE emissions
- `412M` → `hype.emissions.tokens.remaining` → remaining/REPORT_SNAPSHOT → UNKNOWN/UNKNOWN → CGPT_CURSOR (MAPPED_CANONICAL)
- `412.44M` → `hype.emissions.tokens.remaining` → remaining/REPORT_SNAPSHOT → HYPE Stage-1 evidence/2026-08-13T06:03:17Z → CGPT_CURSOR (MAPPED_CANONICAL)
- `~412M remaining` → `hype.emissions.tokens.remaining` → remaining/REPORT_SNAPSHOT → HYPE Stage-1 evidence/2026-08-13 → CGPT_CURSOR (MAPPED_CANONICAL)

## RENDER frames
- `78.90M` → `render.usage.frames.cumulative` → cumulative/REPORT_SNAPSHOT → Render Foundation/2026-08-25T06:43:03Z → CGPT_CURSOR (MAPPED_CANONICAL)

## RENDER BME
- `ratio ~0.21` → `render.bme.ratio.last4` → last4/REPORT_SNAPSHOT → Foundation API/2026-08-25T06:43:03Z → CGPT_CURSOR (MAPPED_CANONICAL)
- `0.21` → `render.bme.ratio.last4` → last4/REPORT_SNAPSHOT → Foundation API/2026-08-25T06:43:03Z → CGPT_CURSOR (MAPPED_CANONICAL)

## PUMP siren watched
- `150` → `pump.siren.watched_wallet_count.current` → current/WALLET_SNAPSHOT → UNKNOWN/UNKNOWN → GROK (WALLET_OWNED)

## RENDER siren watched
- `40` → `render.siren.watched_wallet_count.current` → current/WALLET_SNAPSHOT → UNKNOWN/UNKNOWN → GROK (WALLET_OWNED)

## NOS siren watched
- `21` → `nos.siren.watched_wallet_count.current` → current/WALLET_SNAPSHOT → UNKNOWN/UNKNOWN → GROK (WALLET_OWNED)

## ORCA siren watched
- `6` → `orca.siren.watched_wallet_count.current` → current/WALLET_SNAPSHOT → UNKNOWN/UNKNOWN → GROK (WALLET_OWNED)
